...
class OV5647 : CSI_Sensor
  ...
  def init()
    ...
    # 0=800x640, 1=1280x960, 2=1080p(ESP/RAW10), 3=1080p(Cypress/RAW8)
    self.resolutions = [
      /-> self.regs_800x640_raw8(),
      /-> self.regs_1280x960_raw10(),
      /-> self.regs_1280x720_raw8(),
      /-> self.regs_1296x972_raw8()
    ]
  end
  ...
  # Mode 8: 1280x720 (Output Crop Strategy)
  # Input Window = Full Array (Same as Mode 5)
  # Output Size = 1280x720
  def regs_1280x720_raw8()
    print("OV5647: Config 1280x720 RAW8 (Output Crop)")
    self.width = 1280
    self.height = 720
    self.format = 0  # RAW8
    self.mipi_clock = 200 
    ...
    return [
      # === EVERYTHING SAME AS MODE 5 (1296x972) ===
      ...
      # === WINDOW (FULL ARRAY - SAME AS MODE 5) ===
      [0x3800,0x00], [0x3801,0x00], 
      [0x3802,0x00], [0x3803,0x00], 
      [0x3804,0x0a], [0x3805,0x3f], 
      [0x3806,0x07], [0x3807,0xa1], 
      
      # === OUTPUT SIZE (ONLY CHANGE) ===
      # Tell sensor to only send 1280x720 pixels
      [0x3808,0x05], [0x3809,0x00], # W=1280
      [0x380a,0x02], [0x380b,0xd0], # H=720
      
      # Offsets
      [0x3810,0x00], [0x3811,0x09], [0x3812,0x00], [0x3813,0x00],
  ...
  # Mode 5: 1296x972 RAW8 (2x2 Binned Full FOV)
  def regs_1296x972_raw8()
    print("OV5647: Config 1296x972 RAW8 (2x2 Binned)")
    self.width = 1296
    self.height = 972
    self.format = 0  # RAW8
    # Use standard 800x640 clock (200 Mbps lane) as starting point
    self.mipi_clock = 200 
    ...
    return [
      ...
      # === FORMATTING ===
      # Enable Binning (Bit 0=1) + Subsampling (Bit 4=1, Bit 5=1)
      # 0x31 = 0011 0001
      [0x3814,0x31], [0x3815,0x31], 
      
      # Orientation (Flip/Mirror + Binning)
      [0x3820,0x41], [0x3821,0x03], 
      ...
      # === TIMING ===
      # HTS = 1896 (Same as 800x640)
      [0x380c,0x07], [0x380d,0x68], 
      # VTS = 984 (Same as 800x640 - might need increasing for taller frame?)
      # 972 active lines fits in 984 VTS.
      [0x380e,0x03], [0x380f,0xd8], 
      
      # === WINDOW (Full FOV Binned) ===
      # 2592 / 2 = 1296. 1944 / 2 = 972.
      [0x3800,0x00], [0x3801,0x00], # X Start 0
      [0x3802,0x00], [0x3803,0x00], # Y Start 0
      
      [0x3804,0x0a], [0x3805,0x3f], # X End 2623 (Native coords)
      [0x3806,0x07], [0x3807,0xa1], # Y End 1953 (Native coords)
      
      # Output Size
      [0x3808,0x05], [0x3809,0x10], # W=1296 (0x510)
      [0x380a,0x03], [0x380b,0xcc], # H=972 (0x3CC)
