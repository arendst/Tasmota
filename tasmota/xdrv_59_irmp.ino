/*
  xdrv_59_irmp.ino - complete integration of IRMP for Tasmota
...
enum Xdrv59_IrErrors { IE_NO_ERROR, IE_INVALID_JSON, IE_SYNTAX_IRSND, IE_UNKNOWN };

static const char Xdrv59_IrSendCommands[] PROGMEM = "|" D_CMND_IRSND ;

static void (* const Xdrv59_IrSendCommand[])(void) PROGMEM = {
  &Xdrv59_IrSendCmnd };
...
uint8_t Xdrv59_IrSendIrSendCmndJson(void)
{
  ...
  {
#if IRMP_PROTOCOL_NAMES == 1
    value = Xdrv59_ProtocolName2Id(var_proto.getStr());
#else
    return IE_SYNTAX_IRSND;
#endif
  }
  if (Xdrv59_Validate(value) == false || value == 0 || value > IRMP_N_PROTOCOLS)
    return IE_SYNTAX_IRSND;
  ...
  if (Xdrv59_Validate(value) == false || value > USHRT_MAX)
    return IE_SYNTAX_IRSND;
  ...
  if (Xdrv59_Validate(value) == false || value > UCHAR_MAX)
    return IE_SYNTAX_IRSND;
...
void Xdrv59_IrSendCmndResponse(uint32_t error)
{
  switch (error)
  {
...
void Xdrv59_IrSendCmnd(void)
{
  uint8_t error = IE_SYNTAX_IRSND;

  if (XdrvMailbox.data_len)
  {
    error = Xdrv59_IrSendIrSendCmndJson();
  }
  Xdrv59_IrSendCmndResponse(error);
}
  ...
  if (PinUsed(GPIO_IRSEND) || PinUsed(GPIO_IRRECV))
  {
    switch (function)
    {
      case FUNC_PRE_INIT:
        ...
        {
          Xdrv59_IrReceiveInit();
        }
        break;
      case FUNC_EVERY_50_MSECOND:
        if (PinUsed(GPIO_IRRECV))
        {
          Xdrv59_IrReceiveCheck();  // check if there's anything on IR side
        }
        break;
      case FUNC_COMMAND:
        if (PinUsed(GPIO_IRSEND))
        {
          result = DecodeCommand(Xdrv59_IrSendCommands, Xdrv59_IrSendCommand);
        }
        break;
    }
  }
