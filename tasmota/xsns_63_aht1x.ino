...
const char HTTP_SNS_DEWPOINT[] PROGMEM = "{s}%s " D_DEWPOINT "{m}%s&deg;%c{e}";
...
float CalcTemHumToDew(float h, float t)
{
  float gamma = log(h / 100) + 17.62 * t / (243.5 + t);
  float c = (243.5 * gamma / (17.62 - gamma));
  float result = c;
...
void AHT1XShow(bool json)
{
  for (uint8_t i = 0; i < aht1x_count; i++)
  {
    ...
    else
    {
      ...
      WSContentSend_PD( HTTP_SNS_DEWPOINT, types, dewpoint,TempUnit());
 #endif  // USE_WEBSERVER
      }
   }
}
