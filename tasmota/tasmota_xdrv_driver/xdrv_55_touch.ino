                     ...
                     uint16_t *raw_x, uint16_t *raw_y)
{
  if (TSGlobal.external_ts || FT5206_found || XPT2046_found || CST816S_found)
  {
    ...
    if (raw_x)
    {
      *raw_x = TSGlobal.raw_touch_xp;
    }
    if (raw_y)
    {
      *raw_y = TSGlobal.raw_touch_yp;
    }
    if (gesture)
    {
      *touches = TSGlobal.gesture;
    }
    return true;
  }
  return false;
...
bool Xdrv55(uint32_t function)
{
  ...
  case FUNC_EVERY_100_MSECOND:
    if (FT5206_found || XPT2046_found || GT911_found || CST816S_found || SRES_found)
    {
