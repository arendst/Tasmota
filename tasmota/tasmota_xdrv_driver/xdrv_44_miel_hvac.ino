...
static const struct miel_hvac_map miel_hvac_fan_map[] = {
	...
	{MIEL_HVAC_UPDATE_FAN_QUIET, "quit"},
...
static const struct miel_hvac_map miel_hvac_widevane_map[] = {
	...
	{MIEL_HVAC_UPDATE_WIDEVANE_R, "right_middle"},
	{MIEL_HVAC_UPDATE_WIDEVANE_RR, "right"},
...
struct miel_hvac_softc
{
	...
	struct miel_hvac_msg_update sc_update;
	struct miel_hvac_msg_remotetemp sc_remotetemp;
};
...
static inline void
miel_hvac_send_update(struct miel_hvac_softc *sc, const struct miel_hvac_msg_update *update)
{
	miel_hvac_send(sc, MIEL_HVAC_H_TYPE_UPDATE, update, sizeof(*update));
}

static inline void
miel_hvac_send_remotetemp(struct miel_hvac_softc *sc, const struct miel_hvac_msg_remotetemp *remotetemp)
{
	miel_hvac_send(sc, MIEL_HVAC_H_TYPE_UPDATE, remotetemp, sizeof(*remotetemp));
}
...
miel_hvac_set_power(struct miel_hvac_softc *sc)
{
	...
	update->power = (XdrvMailbox.index & (1 << sc->sc_device)) ? MIEL_HVAC_UPDATE_POWER_ON : MIEL_HVAC_UPDATE_POWER_OFF;
...
miel_hvac_cmnd_sethamode(void)
{
	...
	update->flags |= htons(MIEL_HVAC_UPDATE_F_POWER) | htons(MIEL_HVAC_UPDATE_F_MODE);
...
miel_hvac_publish_settings(struct miel_hvac_softc *sc)
{
	...
	name = miel_hvac_map_byval(set->widevane & MIEL_HVAC_SETTTINGS_WIDEVANE_MASK, miel_hvac_widevane_map, nitems(miel_hvac_widevane_map));
	if (name != NULL)
	{
		ResponseAppend_P(PSTR(",\"" D_JSON_IRHVAC_SWINGH "\":\"%s\""), name);
	}

	ResponseAppend_P(PSTR(",\"Bytes\":\"%s\""), ToHex_P((uint8_t *)&sc->sc_settings, sizeof(sc->sc_settings), hex, sizeof(hex)));
...
miel_hvac_pre_init(void)
{
	...
	{
		AddLog(LOG_LEVEL_ERROR, PSTR(MIEL_HVAC_LOGNAME ": unable to begin serial "
													   "(baudrate %d)"),
			   baudrate);
		goto del;
	}
...
miel_hvac_sensor(struct miel_hvac_softc *sc)
{
	...
	{
		const struct miel_hvac_data_roomtemp *rt = &sc->sc_temp.data.roomtemp;
		char room_temp[33];
		if (rt->temp05 == 0)
		{
	...
	{
		const struct miel_hvac_data_status *s = &sc->sc_status.data.status;
	...
	ResponseAppend_P(PSTR(","
						  "\"Bytes\":\"%s\""),
					 ToHex_P((uint8_t *)&sc->sc_settings, sizeof(sc->sc_settings), hex, sizeof(hex)));
	...
	if (sc->sc_status.type != 0)
	{
		ResponseAppend_P(PSTR(","
							  "\"status\":\"%s\""),
						 ToHex_P((uint8_t *)&sc->sc_status, sizeof(sc->sc_status), hex, sizeof(hex)));
	}
	...
	if (sc->sc_stage.type != 0)
	{
		ResponseAppend_P(PSTR(","
							  "\"stage\":\"%s\""),
						 ToHex_P((uint8_t *)&sc->sc_stage, sizeof(sc->sc_stage), hex, sizeof(hex)));
	}
...
miel_hvac_tick(struct miel_hvac_softc *sc)
{
	...
	{
		struct miel_hvac_msg_remotetemp *remotetemp = &sc->sc_remotetemp;
