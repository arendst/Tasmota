...
void ModbusBridgeHandle(void)
{
  ...
  if (data_ready)
  {
    ...
    AddLogBuffer(LOG_LEVEL_DEBUG_MORE, buffer, tasmotaModbus->ReceiveCount());

    if (error)
    {
      AddLog(LOG_LEVEL_DEBUG, PSTR("ORNO: MODBUSBRIDGE error %d"), error);
    }
    else if (modbusBridge.deviceAddress == 0)
    {
      AddLog(LOG_LEVEL_DEBUG, PSTR("ORNO: MODBUSBRIDGE error, not expecting data."));
    }
    else if (modbusBridge.deviceAddress != (uint8_t)buffer[0])
    {
      AddLog(LOG_LEVEL_DEBUG, PSTR("ORNO: MODBUSBRIDGE error, device address incorrect %d!=%d"), modbusBridge.deviceAddress, buffer[0]);
    }
    else if ((uint8_t)modbusBridge.functionCode != (uint8_t)buffer[1])
    {
      AddLog(LOG_LEVEL_DEBUG, PSTR("ORNO: MODBUSBRIDGE error, function code incorrect %d!=%d"), modbusBridge.functionCode, buffer[1]);
    }
    else if ((uint8_t)modbusBridge.registerCount != (uint8_t)buffer[2])
    {
      AddLog(LOG_LEVEL_DEBUG, PSTR("ORNO: MODBUSBRIDGE error, length incorrect %d!=%d"), modbusBridge.registerCount, buffer[2]);
    }
    else
    {
      AddLog(LOG_LEVEL_DEBUG, PSTR("ORNO: MODBUSBRIDGE received data. Length=%d"), tasmotaModbus->ReceiveCount());
...
void ModbusBridgeInit(void)
{
  ...
  if (result)
  {
    if (2 == result)
    {
      AddLog(LOG_LEVEL_DEBUG, PSTR("ORNO: ModbusBridge HW serial init 8E1 at %d baud"), MBR_SPEED);
      Serial.begin(MBR_SPEED, SERIAL_8E1);
      ClaimSerial();
    }
    else
    {
      AddLog(LOG_LEVEL_DEBUG, PSTR("ORNO: ModbusBridge SW serial init 8E1 at %d baud"), MBR_SPEED);
    }
  }
}
...
void CmndModbusBridgeSend(void)
{
  ...
  // modbusread { "deviceaddress": 1, "startregister": "holding", "address": 1, "length": 1, "endian": "msb", "type": "uint"}
  ...
  const char *sendian = root.getStr(PSTR(D_JSON_MODBUS_ENDIAN), "msb");
  modbusBridge.count = root.getUInt(PSTR(D_JSON_MODBUS_COUNT), 1);

  if (modbusBridge.deviceAddress == 0)
  {
    AddLog(LOG_LEVEL_DEBUG, PSTR("ORNO: ModbusBridge Wrong deviceAddress %d specified"), modbusBridge.deviceAddress);
    return;
  }
  if (modbusBridge.startAddress == 0)
  {
    AddLog(LOG_LEVEL_DEBUG, PSTR("ORNO: ModbusBridge Wrong startAddress %d specified"), modbusBridge.startAddress);
    return;
  }

  if (functionCode > 4) // Writing registers is not supported at this moment
    return;
  ...
  {
    AddLog(LOG_LEVEL_DEBUG, PSTR("ORNO: ModbusBridge Wrong functioncode %d specified"), modbusBridge.functionCode);
    return;
  }

  modbusBridge.endian = ModbusBridgeEndian::mb_undefined;
  if (strcmp(sendian, "msb") == 0)
    modbusBridge.endian = ModbusBridgeEndian::mb_msb;
  if (strcmp(sendian, "lsb") == 0)
    modbusBridge.endian = ModbusBridgeEndian::mb_lsb;
  if (modbusBridge.endian == ModbusBridgeEndian::mb_undefined)
  {
    AddLog(LOG_LEVEL_DEBUG, PSTR("ORNO: ModbusBridge Wrong endian %s specified"), modbusBridge.endian);
    return;
  }
  ...
  if (modbusBridge.type == ModbusBridgeType::mb_undefined)
  {
    AddLog(LOG_LEVEL_DEBUG, PSTR("ORNO: ModbusBridge Wrong type %s specified"), stype);
    return;
  }

  AddLog(LOG_LEVEL_DEBUG, PSTR("ORNO: ModbusBridge Sending: Addr:%d Func:%d Reg:%d Length:%d Type:%d Count:%d"), modbusBridge.deviceAddress, modbusBridge.functionCode, modbusBridge.startAddress, modbusBridge.registerCount, modbusBridge.type, modbusBridge.count);
...
void CmndModbusBridgeWrite(void)
{
  // Todo
  ResponseCmndDone();
}
