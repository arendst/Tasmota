...
#define D_JSON_FLOWRATEMETER_COUNT          "nbImpulsions"
...
// false -> moyenne sur 'FLOWRATEMETER_WEIGHT_AVG_SAMPLE' mesures
// true -> valeur brute instantanée
//bool flowratemeter_raw_value = true;
...
/* 
@ flowratemeter_period -> delai (microS) entre 2 interruptions
@ flowratemeter_last_irq -> timer (microS)
@ flowratemeter_period_duration -> somme des délai 'flowratemeter_period' (microS/100)

@flowratemeter_period_sum -> compteur de passage donc de nombre d'impulsions
@flowratemeter_period_sum_dT -> enregistre le timer (milliS)
*/
...
void IRAM_ATTR FlowRateMeter3IR(void) {
  FlowRateMeterIR(2);
}
void IRAM_ATTR FlowRateMeter4IR(void) {
  FlowRateMeterIR(3);
}
void IRAM_ATTR FlowRateMeter5IR(void) {
  FlowRateMeterIR(4);
}
void IRAM_ATTR FlowRateMeter6IR(void) {
  FlowRateMeterIR(5);
}

// Reset des compteurs à minuit
/* 
@flowratemeter_period_sum -> compteur de passage donc de nombre d'impulsions totale sur la journée
@flowratemeter_period_sum_dT -> enregistre le timer (milliS)
*/
...
/*
@ FLOWRATEMETER_WEIGHT_AVG_SAMPLE -> nombre de mesure pour etablir la moyenne
@ flowratemeter_count -> nombre de mesures effectuées pour chaque débitmètre
@ flowratemeter_period_avg -> debit moyenné sur la période pour chaque débitmètre
...
/*
@ meter -> Indice du débitmètre concerné
@ rate_float -> débit calculé
@ amount_today -> consommation journalière
@ _flowratemeter_period -> periode de calcul pour la journée
...
void FlowRateMeterGetValue(uint32_t meter, float *rate_float, float *amount_today) {
  ...
  if (Settings->SensorBits1.flowratemeter_test) {
    Settings->SensorBits1.flowratemeter_raw_value ? flowratemeter_period[meter] = random(0, 100000) : flowratemeter_period_avg[meter] = random(0, 100000);
    bool_flowratemeter_test = true;
  } else {
    if (bool_flowratemeter_test) {
      for (uint32_t i = 0; i < MAX_FLOWRATEMETER; i++) {
        if (PinUsed(GPIO_FLOWRATEMETER_IN, i)) {
          Settings->SensorBits1.flowratemeter_raw_value ? flowratemeter_period[i] = 0 : flowratemeter_period_avg[i] = 0;  
        }
      }
      bool_flowratemeter_test = false;  
    }
  }
...
void FlowRateMeterShow(bool json) {
  ...
  //Serial.print("size : ");    Serial.println(sizeof(TSettings));
  ...
  if (json) {
    ...
    for (uint32_t i = 0; i < MAX_FLOWRATEMETER; i++) {
      if (PinUsed(GPIO_FLOWRATEMETER_IN, i)) {
        ...
        uint32_t count = (Settings->SensorBits1.flowratemeter_raw_value ? flowratemeter_period[i] : flowratemeter_period_avg[i]);
        ...
        ResponseAppend_P(PSTR("\"" D_JSON_FLOWRATEMETER_DURATION_TODAY "\":%ld"),
          flowratemeter_period_duration[i] / 10000);
    ...
    //Settings->SensorBits1.flowratemeter_raw_value
    // head
