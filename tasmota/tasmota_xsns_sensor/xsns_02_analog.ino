...
void AdcInit(void) {
  ...
  for (uint32_t i = 0; i < MAX_ADCS; i++) {
    ...
    if (PinUsed(GPIO_ADC_BATTERY_DIODE, i)) {
      AdcAttach(Pin(GPIO_ADC_BATTERY_DIODE, i), ADC_BATTERY_DIODE);
    }
...
void AdcShow(bool json) {
  ...
  for (uint32_t idx = 0; idx < Adcs.present; idx++) {
    ...
    switch (Adc[idx].type) {
      case ADC_BATTERY: {
        float analog = (float)(AdcRead(Adc[idx].pin, 5))/100;
