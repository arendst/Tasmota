/*
  xsns_81_TOF10120.ino - TOF10120 sensor support for Tasmota
...
uint16_t Tof10120Read() {
  ...
  if (Wire.available() == 2) {
    ...
    distance |= Wire.read();
      ...
      AddLog_P(LOG_LEVEL_DEBUG, "TOF10120: no value read");
      return(TOF10120_INCORRECT_DISTANCE); 
    } 
  AddLog_P(LOG_LEVEL_DEBUG, "TOF10120: buffer %u ", distance);
  return(distance);
}
...
#ifdef USE_WEBSERVER
const char HTTP_SNS_TOF10120[] PROGMEM =
 "{s}TOF10120 " D_DISTANCE "{m}%d" D_UNIT_MILLIMETER "{e}"; // {s} = <tr><th>, {m} = </th><td>, {e} = </td></tr>
#endif  // USE_WEBSERVER
...
void Tof10120Show(bool json) {
  if (json) {
    ...
    WSContentSend_PD(HTTP_SNS_TOF10120, tof10120_sensor.distance);
#endif
  }
}
