...
#if defined(USE_FT5206) || defined(USE_GT911) || defined(USE_CST816S)
    cp = strstr(ddesc, ":TI");
    if (cp)
    {
      uint8_t wire_n = 1;
...
#ifdef USE_CST816S
        if (!wire_n)
          CST816S_Touch_Init(&Wire, irq, rst);
        else
          CST816S_Touch_Init(&Wire1, irq, rst);
#endif
      }
      else
      {
...
#endif // USE_FT5206 || GT911 || CST816S
