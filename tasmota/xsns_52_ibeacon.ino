...
void ESP32ScanTask(void *pvParameters){
  ...
  ESP32BLEScan->start(0, ESP32scanEndedCB, true); // never stop scanning, will pause automatically while connecting

  uint32_t timer = 0;
  for(;;){
    if(ESP32BLE.mode.shallClearResults){
      ESP32BLEScan->clearResults();
      ESP32BLE.mode.shallClearResults=0;
    }
