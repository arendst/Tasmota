/*
  xdrv_48_BLE_ESP32.ino - BLE via ESP32 support for Tasmota
...
// seen devices
#define MAX_BLE_DEVICES_LOGGED 40
...
const char kBLE_Commands[] PROGMEM = D_CMND_BLE "|"
  "Period|Adv|Op|Mode|Details|Scan|Alias|Name|Debug|Devices|MaxAge";
...
int addSeenDevice(const uint8_t *mac, const char *name, int8_t RSSI){
  int res = 0;
...
#define MAX_DEV_JSON_ALIAS_LEN 20
...
const char HTTP_BLE_DEV_START[] PROGMEM =
  ...
  "<tr><th><label>mac</label></th><th><label>alias</label></th><th><label>name</label></th><th><label>RSSI</label></th><th><label>Age(max)</label></th></tr>";
const char HTTP_BLE_DEV[] PROGMEM =
  "<tr><td><label>%s</label></td><td><label>%s</label></td><td><label>%s</label></td><td><label>%d</label></td><td><label>%d(%d)</label></td></tr>";
...

  {
    ...
    if (seenDevices.size()){
      ...
      for (int i = 0; i < seenDevices.size(); i++){
        ...
        WSContentSend_P(HTTP_BLE_DEV, addr, alias, dev->name, dev->RSSI, ageS, dev->maxAge);
      }
