...
void CCS811ReadMailboxValue( uint8_t address, uint8_t mailbox, byte * pbuf, uint8_t buflen)
{
  ...
  for (uint8_t i = 0; i < buflen; i++) {
    *(pbuf + i) = Wire.read();
#ifdef CCS811_DEBUG
    AddLog_P(LOG_LEVEL_DEBUG, PSTR( D_LOG_DEBUG D_CMND_CCS811 " reading byte %u: 0%02x / %u"), i, *(pbuf + i), *(pbuf + i));
#endif
  }
}
...
#ifdef CCS811_DEBUG
  for (uint8_t i = 0; i < buflen; i++) {
    AddLog_P(LOG_LEVEL_DEBUG, PSTR( D_LOG_DEBUG D_CMND_CCS811 " writing byte %u: 0%02x / %u"), i, *(pbuf + i), *(pbuf + i));
  }
#endif
	Wire.beginTransmission(address);
  ...
  if ((!strncasecmp_P(XdrvMailbox.topic, PSTR(D_CMND_CCS811), prefix_len)) &&
    ...
    for (i = 0, pccsd = ccsd; i < MAXDEVICECOUNT; i++, pccsd++) {
      if  (pccsd->device_index == XdrvMailbox.index) {
        pccsd_command = pccsd;
#ifdef CCS811_DEBUG
        AddLog_P(LOG_LEVEL_DEBUG, PSTR( D_LOG_DEBUG D_CMND_CCS811 " I2C Address: 0%02x"), pccsd_command->address);
#endif
        break;
      }
    }
