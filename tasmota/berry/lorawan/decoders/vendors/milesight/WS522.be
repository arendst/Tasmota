...
class LwDecoWS522
  static def decodeUplink(Name, Node, RSSI, FPort, Bytes)
    ...
    if valid_values
      ...
      global.ws522Nodes.insert(Node, [Name, Node, last_seen, rssi, voltage, active_power, power_factor, energy_sum, current, button_state, voltage_ls, active_power_ls, power_factor_ls, energy_sum_ls, current_ls, button_state_ls])
    end
  ...
  static def add_web_sensor()
    ...
    for sensor: global.ws522Nodes
      ...
      var rssi = sensor[3]
      var battery_last_seen = last_seen # no battery present, it's a plug
      var battery = 100                 # always 100% (may be better to remove the battery symbol with a cheat code?)
