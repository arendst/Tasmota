# LwDecode Framework for LoRaWAN Device Decoders

## Overview

LwDecode is a Berry framework for Tasmota ESP32 that provides a standardized interface for decoding LoRaWAN sensor payloads. It integrates with Tasmota's LoRaWAN bridge functionality to decode device uplinks, display sensor data in the web UI, and manage downlink commands.
...
2. **Device Decoders** - Individual `.be` files implementing sensor-specific logic:
   - Located in `/vendors/<vendor>/<model>.be`
...
### Manual Installation
1. Upload decoder to `/vendors/<vendor>/<model>.be`
2. Configure in console:
   ```
   LoRaWanAppKey1 <32-char-key>
   LoRaWanDecoder1 vendors/vendor/model.be
