...
#
# Official Links
# - Homepage:  https://www.milesight.com/iot/product/lorawan-sensor/ws301
# - Userguide: https://resource.milesight-iot.com/milesight/iot/document/ws301-user-guide.pdf
...
class LwDecode_WS301
    ...
    def decodeUplink(name, node, rssi, fport, payload)
        ...
        try
            ...
            # Decode based on fport 85 (WS301 uses single port)
            if fport == 85
                var i = 0
                while i < size(payload)
                    ...
                    # Door State (0x03, 0x00) - 1 byte
                    elif channel_id == 0x03 && channel_type == 0x00 && i < size(payload)
                        data['door_open'] = payload[i] == 0x01
                        data['door_text'] = payload[i] == 0x01 ? "OPEN" : "CLOSED"
                        i += 1
                        
                    # Tamper Detection (0x04, 0x00) - 1 byte
                    elif channel_id == 0x04 && channel_type == 0x00 && i < size(payload)
                        data['tamper_detected'] = payload[i] == 0x01
                        i += 1
                        
                    # Install Status (0x05, 0x00) - 1 byte
                    elif channel_id == 0x05 && channel_type == 0x00 && i < size(payload)
                        data['install_status'] = payload[i] == 0x01
                        data['install_text'] = payload[i] == 0x01 ? "INSTALLED" : "NOT INSTALLED"
                        i += 1
            ...
            print(f"LwDecode_WS301 error: {m}")
            return nil
        end
    end
    ...
    def add_web_sensor()
        ...
        var name_tooltip = "Milesight WS301 Magnetic Contact"
        ...
        # Build compact door sensor display
        fmt.start_line()
        
        # Door state with dynamic emoji
        if data_to_show.contains('door_open')
            var door_emoji = data_to_show['door_open'] ? "🔓" : "🔒"
            var door_text = data_to_show.find('door_text', "UNKNOWN")
        ...
        # Add alert line for tamper/install/reset events
        var has_alerts = false
        if data_to_show.find('tamper_detected', false) ||
           data_to_show.find('install_status', true) == false ||
           data_to_show.find('device_reset', false)
            fmt.start_line()
            has_alerts = true
            
            if data_to_show.find('tamper_detected', false)
                fmt.add_status("Tamper", "⚠️", "Tamper Detected")
            end
            
            if data_to_show.find('install_status', true) == false
                fmt.add_status("Not Installed", "❌", "Installation Error")
            elif data_to_show.find('install_status', false) == true
                fmt.add_status("Installed", "✅", "Installation OK")
            end
    ...
    def register_downlink_commands()
        ...
        tasmota.remove_cmd("WS301Interval")
        tasmota.add_cmd("WS301Interval", def(cmd, idx, payload_str)
        ...
        # Device Reboot
        tasmota.remove_cmd("WS301Reboot")
        tasmota.add_cmd("WS301Reboot", def(cmd, idx, payload_str)
            var hex_cmd = "FF10FF"
            return lwdecode.SendDownlink(global.WS301_nodes, cmd, idx, hex_cmd)
        end)
...
tasmota.remove_cmd("WS301TestPayload")
tasmota.add_cmd("WS301TestPayload", def(cmd, idx, payload_str)
    # Parse hex string to bytes
    var test_payload = bytes(payload_str)
    
    # Force driver load by LwDecode framework
    var result = LwDeco.decodeUplink("TestWS301", "test_node", -85, idx, test_payload)
...
tasmota.remove_cmd("WS301NodeStats")
tasmota.add_cmd("WS301NodeStats", def(cmd, idx, node_id)
...
tasmota.remove_cmd("WS301ClearNode")
tasmota.add_cmd("WS301ClearNode", def(cmd, idx, node_id)
