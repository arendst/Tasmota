...
void MqttInit(void) {
  ...
  void ProvisionAzureDPS(){
    ...
    String dPSHostName = endpoint;
    dPSHostName.replace("https://", "");
    dPSHostName.replace("/", "");
    ...
    httpsClient.begin(*tlsClient, dPSURL);
    httpsClient.addHeader("Host", dPSHostName);
...
//  EspClient = WiFiClient();               // Wifi Client reconnect issue 4497 (https://github.com/esp8266/Arduino/issues/4497)
  ...
  if (azureMqtt_password.indexOf("SharedAccessSignature") == -1) {
    ...
    azureMqtt_password = AzurePSKtoToken(SettingsText(SET_MQTT_HOST), SettingsText(SET_MQTT_CLIENT), SettingsText(SET_MQTT_PWD), 3600);
  }

  String azureMqtt_userString = String(SettingsText(SET_MQTT_HOST)) + "/" + String(SettingsText(SET_MQTT_CLIENT)); + "/?api-version=2018-06-30";
  if (MqttClient.connect(TasmotaGlobal.mqtt_client, azureMqtt_userString.c_str(), azureMqtt_password.c_str())) {
#else
