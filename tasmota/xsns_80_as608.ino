...
void as608init(){
    ...
    if (PinUsed(GPIO_AS608_RX) && PinUsed(GPIO_AS608_TX)){
        ...
        if (finger->verifyPassword()){
          ...
          as608selected = true;
          AddLog_P2(LOG_LEVEL_INFO, PSTR("AS60x: Detected. %i fingerprints stored."), finger->templateCount);
        }else{
          AddLog_P2(LOG_LEVEL_INFO, PSTR("AS60x: not found! Got data: %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X"), finger->mydata[0], finger->mydata[1], finger->mydata[2], finger->mydata[3], finger->mydata[4], finger->mydata[5], finger->mydata[6], finger->mydata[7], finger->mydata[8], finger->mydata[9], finger->mydata[10], finger->mydata[11]);
...
void as608Main(){
    ...
    if (!enrollstep){
        ...
        // found a match!
        AddLog_P2(LOG_LEVEL_INFO, PSTR("AS60x: Finger #%i found with confidence of %i"), finger->fingerID, finger->confidence);
        TasmotaGlobal.mqtt_data[0] = '\0';
        snprintf_P(TasmotaGlobal.mqtt_data, sizeof(TasmotaGlobal.mqtt_data), PSTR("{\"AS680_Finger\":%i, AS608_Confidence\":%i}"), finger->fingerID, finger->confidence);
    ...
    if (XSNS_80 == XdrvMailbox.index) {
        ...
        if (!strcmp(subStr(sub_string, XdrvMailbox.data, ",", 1),"enroll")) {
          if (!enrollstep){
                ...
                AddLog_P2(LOG_LEVEL_INFO, PSTR("AS60x: Enroll called #%i"), ModellNumber);
                TasmotaGlobal.mqtt_data[0] = '\0';
        ...
        if (!strcmp(subStr(sub_string, XdrvMailbox.data, ",", 1),"getNumber")) { // Note 1 used for param number
          finger->getTemplateCount();
          AddLog_P2(LOG_LEVEL_INFO, PSTR("AS60x: %i fingerprints stored."), finger->templateCount);
