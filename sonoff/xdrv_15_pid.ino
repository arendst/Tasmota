/*
  xdrv_15_pid.ino - PID Controller Algorithm Support for Sonoff-Tasmota
...
/**
 * This extension adds a PID (Proportional Integral Derivative) feature
 * into the Sonoff-Tasmota software.
 *
 * The PID algorithm is one designed to be used to control real-world processes
 * that the users of Sonoff devices are likely to encounter. This includes room
 * heating/cooling, temperature control when brewing, and a multitude of other
 * processes. The PID tuning parameters are designed to be meaningful in the
 * real world (rather than the abstract Ki Kd Kp that are often used which
 * are completely meaningless to most). The algorithm is based on that in the
 * node-red node node-red-contrib-pid which has been well received.
 *
 * In use it can either regularly be given the current process value via MQTT
 * or if the device has a sensor attached then that sensor can be used to read
 * the process value. So using a TH10 with a DS18B20 the complete PID loop
 * control can be build into the device so that the process will continue to
 * be controlled even if the wifi is down. This is a very cost effective way
 * of achieving PID control.
 *
 * The algorithm allows the relay to be used in a time proportioned way using
 * the Time Proportioned output extension.
 *
 * The loop tuning parameters can be set at build time and can be adjusted at
 * run time via MQTT.
 *
 * The PID code adds 4.3k
 *
 * The ESP8266 will run the PID algorithm at 1 cycle per second, which is much
 * faster than is needed for the sort of processes Sonoff devices are usually
 * associated with. It rather clobbers the Sonoff-Tasmota terminal output in
 * the web browser at that rate so it is getting near to the limit. The maximum
 * anyone is likely to need it running at is maybe once every 5 seconds, and
 * the majority of home IoT applications probably nearer once per minute would
 * be sufficient, so the device is well up to the task.
 *
 ...
 * This is directed towards using the algorithm in the node-red node
 * node-red-contrib-pid but the algorithm here is based on the same code so the
 * tuning technique described there should work just the same.
 *
 */
...
#endif                                         // the gain of the loop and is the range of process value over which
...
#endif                                         // in seconds. It represents the time constant of the integration
...
#endif                                         // in seconds. It represents the time constant of the derivative effect.
...
#endif                                         // to preset the integrated error value when the flow is deployed in
...
#endif                                         // It is provided to cope with unusual situations such as a faulty sensor
...
#endif                                         // the DS18B20), the use of deriviative can be problematic as when the
...
#endif                                         // the control (1=enable, auto mode, 0=disabled, manual mode). When in
...
#endif                                         // each time a new pv value is received, for most applictions specify 0.
...
#endif                                         // process, set this to indicate which timeprop output to use. For a device
...
#endif                                         // this is not required.  The rate that the sensor is read is defined by TELE_PERIOD
