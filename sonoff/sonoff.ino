...
//end
    ...
    if (tele_period >= sysCfg.tele_period) {
      ...
      if (sysCfg.deepsleep > 10) {
        ...
        snprintf_P(mqtt_data, sizeof(mqtt_data), S_OFFLINE);
        mqtt_publish_topic_P(2, PSTR(D_LWT), false);  // Offline or remove previous retained topic

        yield();
        snprintf_P(mqtt_data, sizeof(mqtt_data), PSTR("{\"Time\":\"%s\", \"Uptime_s\":%d}"), getDateTime().c_str(), rtcMem.uptime);
        mqtt_publish_topic_P(2, PSTR("UPTIME_S"));
        yield();
        rtcMem.uptime = 0;
        RTC_Save();
