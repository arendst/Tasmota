...
#define MOD_VERSION_STRING         "modified-1.13.2"    // Would be great to have a macro that fills this from VERSION ...
...
void PublishStatus(uint8_t payload)
{
  ...
  if ((0 == payload) || (1 == payload)) {
    snprintf_P(mqtt_data, sizeof(mqtt_data), PSTR("{\"" D_CMND_STATUS D_STATUS1_PARAMETER "\":{\"" D_JSON_BAUDRATE "\":%d,\"" D_CMND_GROUPTOPIC "\":\"%s\",\"" D_CMND_OTAURL "\":\"%s\",\"" D_JSON_UPTIME "\":\"%s\",\"" D_JSON_STARTUPDATETIMEUTC "\":\"%s\",\"" D_CMND_SLEEP "\":%d,\"" D_JSON_BOOTCOUNT "\":%d,\"" D_JSON_SAVECOUNT "\":%d,\"" D_JSON_SAVEADDRESS "\":\"%X\"}}"),
      baudrate, Settings.mqtt_grptopic, Settings.ota_url, GetUptime().c_str(), GetStartupDateTimeUtc().c_str(), Settings.sleep, Settings.bootcount, Settings.save_flag, GetSettingsAddress());
