...
//STB mod
  CMND_GPIO, CMND_GPIOS, CMND_PWM, CMND_PWMFREQUENCY, CMND_PWMRANGE, CMND_COUNTER, CMND_COUNTERTYPE, CMND_COUNTERDEVIDER,
//end
...
const char kTasmotaCommands[] PROGMEM =
  ...
  //STB mod
  D_CMND_GPIO "|" D_CMND_GPIOS "|" D_CMND_PWM "|" D_CMND_PWMFREQUENCY "|" D_CMND_PWMRANGE "|" D_CMND_COUNTER "|"  D_CMND_COUNTERTYPE "|"   D_CMND_COUNTERDEVIDER "|"
  //end
  ...
  if (Settings.tele_period) {
    ...
    if (tele_period >= Settings.tele_period) {
      ...
      if (Settings.deepsleep > 10 && Settings.deepsleep < 4294967295) {
        ...
        snprintf_P(mqtt_data, sizeof(mqtt_data), S_OFFLINE);
        MqttPublishPrefixTopic_P(2, PSTR(D_LWT), false);  // Offline or remove previous retained topic
...
//end
  for (byte i = 0; i < maxdev; i++) {
    ...
    if (!i && ((SONOFF_DUAL == Settings.module) || (CH4 == Settings.module))) {
      ...
      //STB mod
      if ((pin[GPIO_KEY1 +i] < 99) && !blockgpio0 && i < MAX_KEYS) {
      //end
        button_present = 1;
