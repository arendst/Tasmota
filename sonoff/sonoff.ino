  ...
  if (type != NULL)
  {
    ...
    else if (CMND_BLINKTIME == command_code)
    {
      if ((payload > 1) && (payload <= 3600))
      {
        ...
        if (blink_timer > 0)
        {
          blink_timer = millis() + (100 * payload);
        }
      }
    ...
    else if ((CMND_SETOPTION == command_code) && (index < 82))
    {
      ...
      byte pindex;
      if (index <= 31)
      { // SetOption0 .. 31 = Settings.flag
        ptype = 0;
        pindex = index; // 0 .. 31
      }
      else if (index <= 49)
      { // SetOption32 .. 49 = Settings.param
        ptype = 2;
        pindex = index - 32; // 0 .. 17 (= PARAM8_SIZE -1)
      }
      else
      { // SetOption50 .. 81 = Settings.flag3
        ptype = 1;
...
*/
      ...
      {
        if (2 == ptype)
          snprintf_P(stemp1, sizeof(stemp1), PSTR("%d"), Settings.param[pindex]);
...
void PerformEverySecond()
{
  ...
  {                                   // Microcontroller needs 3 seconds before accepting commands
  ...
  if (status_update_timer)
  {
    ...
    if (!status_update_timer)
    {
      for (byte i = 1; i <= devices_present; i++)
      {
        ...
        {
          break;
        } // Only report status of light relay
      }
    }
  }
