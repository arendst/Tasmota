...
const char HTTP_DEVICE_CONTROL[] PROGMEM = "<td style='width:%d%%'><button  style=\"line-height:5.0rem\" onclick='la(\"?o=%d\");'>%s%s</button></td>";
...
void HandleModuleConfiguration()
{
  ...
  for (byte i = 0; i < MAX_GPIO_PIN; i++) {
    if (GPIO_USER == cmodule.gp.io[i]) {
      ...
      snprintf_P(mqtt_data, sizeof(mqtt_data), PSTR("<tr><td style='width:60%%'>%s <b>" D_GPIO "%d</b> %s</td><td style='width:40%%'><select id='g%d' name='g%d'></select></td></tr>"),
