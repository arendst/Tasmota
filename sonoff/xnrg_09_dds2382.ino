...
void DDS2382Every250ms(void)
{
  ...
  switch (dds2382_state) {
    ...
    case DDS2382_STATE_WAIT_RESPONSE:
      if (DDS2382Modbus->ReceiveReady()) {
        for (uint8_t i=0;i<48;i++) {
            dds2382_rx_buffer[i]=i;
        }        
        error = DDS2382Modbus->ReceiveBuffer(dds2382_rx_buffer,18);
        if (error) {
          AddLog_P2(LOG_LEVEL_INFO, PSTR(D_LOG_DEBUG "dds2382 response error %d"), error);
          for (uint8_t i=0;i<48;i++) {
            AddLog_P2(LOG_LEVEL_INFO, PSTR(D_LOG_DEBUG "%d: %d"), i, dds2382_rx_buffer[i]);
          }
          dds2382_state = DDS2382_STATE_SEND_CMD;
        }
