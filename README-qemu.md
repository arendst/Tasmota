...
`run-it.sh` builds a Tasmota ESP32 firmware with PlatformIO and boots it under an
Espressif QEMU fork on an amd64 host — serial console always, and a **browsable web
UI** when the WiFi-capable fork is used.
...
```bash
NET_WIFI=1 ./run-it.sh        # build -> image -> wifi QEMU -> boot, joins emulated AP
# then browse:
...
```bash
./run-it.sh                   # stock fork, serial console only
```
...
```bash
./run-it.sh decode                 # decodes tasmota-qemu/qemu.log
./run-it.sh decode some-other.log  # decodes a specific log
```
...
Is it QEMU or the firmware? Run `./run-it.sh smoketest`: it builds a minimal Arduino
...
```bash
./run-it.sh fixui          # sends SetOption128 1 (disable referer check)
```
