...
- MiEL HVAC Modbus slave frames incoming requests by length instead of the T3.5 gap, resyncs byte-by-byte on a framing mismatch, waits the RTU turnaround silence before answering, uses a 1 kB RX buffer and skips flush() on auto-direction transceivers - fixes spurious CRC errors and corrupted responses above 9600 baud and on a shared bus
