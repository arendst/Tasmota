...
public:
  WemoSwitch(int deviceId, ESP8266WebServer *webServer)
  {
    ...
    AddLog_P2(LOG_LEVEL_DEBUG, PSTR("\nDEVICE #%d LISTENING ON PORT %d"), _deviceId, _localPort);
  }
  ...
  {
    AddLog_P2(LOG_LEVEL_DEBUG, PSTR("\nWemoRespondToMSearch DEVICE #%d: %d"), _deviceId, echo_type);
    ...
    if (PortUdp.beginPacket(udp_remote_ip, udp_remote_port))
    {
      char type[24];
      if (1 == echo_type)
      { // type1 echo 1g & dot 2g
        strcpy_P(type, URN_BELKIN_DEVICE_CAP);
      }
      else
      { // type2 echo 2g (echo, plus, show)
      ...
      //AddLog_P2(LOG_LEVEL_DEBUG, PSTR("\nSending packet device %d: %s"), _deviceId, response);
...
private:
  ...
  {
    AddLog_P2(LOG_LEVEL_DEBUG, PSTR(D_LOG_HTTP "%s from %s"), msg, _webServer->client().remoteIP().toString().c_str());
  }
  ...
  void HandleUpnpEvent()
  {
    ...
    AddLog_P2(LOG_LEVEL_DEBUG, PSTR("\nHandleUpnpEvent for deviceId %d: %s"), _deviceId, event);
    ...
    snprintf_P(event, sizeof(event), setup_xml.c_str(), state, bitRead(power, _deviceId - 1), state);
  ...
  void HandleUpnpSetupWemo(void)
  {
    ...
    AddLog_P2(LOG_LEVEL_DEBUG, PSTR("\nSENDING DEVICE #%d: %s"), _deviceId, setup_xml.c_str());
  }

public:
  void RegisterHandlers(void)
  {
    AddLog_P2(LOG_LEVEL_DEBUG, PSTR("\nREGISTER DEVICE #%d"), _deviceId);
...
void WemoRespondToMSearch(int echo_type)
{
  ...
  for (int i = 0; i < numOfWemoSwitch; i++)
  {
...
bool Xdrv21(uint8_t function)
{
  ...
  if (devices_present && (EMUL_WEMO == Settings.flag2.emulation))
  {
    ...
    {
    case FUNC_LOOP:
      for (int i = 1; i < numOfWemoSwitch; i++) // Handle devices web server
      {
    ...
    case FUNC_WEB_ADD_HANDLER:
      ...
      AddLog_P2(LOG_LEVEL_DEBUG, PSTR("\nADDING HANDLERS FOR %d DEVICES"), devices_present);
