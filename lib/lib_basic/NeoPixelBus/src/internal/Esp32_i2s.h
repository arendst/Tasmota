...
#if ESP_IDF_VERSION_MAJOR >= 5
static i2s_bus_t I2S[I2S_NUM_AUTO] = {
...
esp_err_t i2sSetClock(uint8_t bus_num, uint8_t div_num, uint8_t div_b, uint8_t div_a, uint8_t bck, uint8_t bits) {
    ...
    // typeof(i2s->tx_clkm_conf) clkm_conf;
    // clkm_conf.val = 0;
    // clkm_conf.tx_clk_sel = 2;
    // clkm_conf.tx_clkm_div_num = div_num;
    // i2s->tx_clkm_conf.val = clkm_conf.val;
