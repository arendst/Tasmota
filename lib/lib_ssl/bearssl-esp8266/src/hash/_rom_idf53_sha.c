/* _sha_rom_idf53.c — BearSSL drop‑in with ESP32 ROM SHA acceleration
 *
 ...
 *  ----------------------------
 *  - The ESP32 ROM SHA‑256 implementation shares internal state with
 *    hardware‑accelerated HMAC and other SHA‑256‑based operations.
 *  - In this “single global context” model, any attempt to run SHA‑256
 *    concurrently (or interleaved with HMAC) will overwrite the ROM’s
 *    internal state, producing incorrect results.
 *  - Because of this, plain SHA‑256 in this scope is only viable when
 *    SOC_SHA_SUPPORT_RESUME is available and the implementation can
 *    save/restore state between calls. Without that, it is unsafe here.
 *
 *  Supported ROM Algorithms (if present in ROM):
 *  ----------------------------------------------
 *    • SHA‑1
 *    • SHA‑224
 *    • SHA‑256  (see limitation above)
 *    • SHA‑384
 *    • SHA‑512
 */
