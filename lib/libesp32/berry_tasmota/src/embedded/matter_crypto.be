...
class Matter_Crypto
  ...
  var spake_M, spake_N    # due to the nature of solification, we can't solidify the bytes versions nor store as static variable

  # init does not do much
  def init()
    self.spake_M = bytes().fromhex(self.spake_M_hex)
    self.spake_N = bytes().fromhex(self.spake_N_hex)
  end
  ...
  def compute_pA(x)
    ...
    var ec = crypto.EC_P256()
    x = ec.mod(x)
    self.pA = ec.muladd(self.w0, self.spake_M, x, bytes() #- empty means generator -#)    # compute x*G+w0*M
  end
end
